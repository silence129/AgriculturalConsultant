...
public class EaseCommonUtils {
     ...
     * @param user
     */
    public static void setUserInitialLetter(ACGroupMemberEntity.MembersBean user) {
        ...
        if ( !TextUtils.isEmpty(user.getNickName()) ) {     // 获取nick
            letter = new GetInitialLetter().getLetter(user.getNickName());
            user.setInitialLetter(letter);
            return;
        } 
        if (letter.equals(DefaultLetter) && !TextUtils.isEmpty(user.getUserName())) {  // 获取 username 获取该Letter
            letter = new GetInitialLetter().getLetter(user.getUserName());
        }
        user.setInitialLetter(letter);  // 设置并保存起来
    }
